...
class Problem0011 {
    ...
    func solve(input: Array2D<Int>) -> Int {
        
        var highestProduct = 0
            
        for group in [input.getHorizontals(), input.getVerticals(), input.getAllLeadingDiagonals(), input.getAllTrailingDiagonals()] {
            for row in group {
                for candidate in row.getSubArraysOfWidth(4) {
                    if candidate.product() > highestProduct {
                        highestProduct = candidate.product()
                    }
                }
            }
        }
